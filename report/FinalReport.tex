...
\begin{document}
	...
	The growth of IoT and smart devices has led to the need for modern, less intrusive notification methods. Traditional notification methods such as constant alerts via phone or email are not only distracting but also easily miss out on important information that needs to be constantly displayed, such as room temperature or time.
	
	
	From that problem, our team decided to develop the Personal Ambient Notifier project. This device acts as a subtle environmental information display board, providing notifications about real-time, temperature and personal messages in an intuitive way, without being annoying like mobile notifications.
	
	
	Although it is a compact device, it brings unique value: through the application on the mobile phone, the Led Matrix can be controlled by bluetooth signal and memory management by EEPROM chip.
	
	In short, the project is not only a display device, but also a low-cost, flexible IOT model, helping users receive environmental information in a smart and personalized way.
